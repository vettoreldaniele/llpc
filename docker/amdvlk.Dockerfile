...
#          echo "export ASAN_OPTIONS=detect_leaks=0" >> env.sh; \
#          echo "export LD_PRELOAD=$(clang -print-file-name=libclang_rt.asan-x86_64.so)" >> env.sh; \
#        else \
...
RUN mkdir /vulkandriver/coverage_report && touch /vulkandriver/coverage_report/test.txt
RUN ls -l /vulkandriver/coverage_report
#RUN which gsutil